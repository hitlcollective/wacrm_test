 ...
 * whether the saved config is healthy. Returns 200 in all non-auth
 * cases so the UI can render an appropriate message rather than
 * show a 500.
 *
 * Provider-aware since PR 2a: dispatches on `whatsapp_config.provider`.
 * The Meta response shape is unchanged so the existing UI keeps
 * working. The Evolution response shape is the same wrapper with a
 * provider discriminator and an `evolution` object carrying the
 * instance state + ownerJid.
 *
 * Meta response (200):
 *   { provider: 'meta', connected: true,  phone_info: {...} }
 *   { provider: 'meta', connected: false, reason: 'no_config', message: '...' }
 *   { provider: 'meta', connected: false, reason: 'token_corrupted', message: '...', needs_reset: true }
 *   { provider: 'meta', connected: false, reason: 'meta_api_error', message: '...' }
 *
 * Evolution response (200):
 *   { provider: 'evolution', connected: true,  evolution: { state, ownerJid, instance_name } }
 *   { provider: 'evolution', connected: false, reason: '...', message: '...' }
 */
 ...
 * Provider-aware since PR 2a: the request body is dispatched on
 * `provider`. Meta path is byte-equivalent with the pre-PR-1 route.
 * Evolution path accepts `{ provider: 'evolution', base_url,
 * instance_name, apikey, webhook_secret? }` and stores the apikey
 * + webhook_secret encrypted, the rest in the clear.
 */
...
async function handleEvolutionPost(
  ...
  const baseRow = {
    ...
    evolution_webhook_url_secret: encryptedWebhookSecret,
 ...
 *
 * Provider-agnostic — the row holds the tenancy key, and the user
 * (or a separate disconnect flow) is responsible for telling
 * Evolution to drop the instance. For Evolution, the dedicated
 * DELETE /api/whatsapp/evolution/instance route does both.
 */
